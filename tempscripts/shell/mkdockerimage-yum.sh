...
# for use localhost yum，rm the yumrepos of default
rm -rf "$target"/etc/yum.repos.d/*
cp -f /etc/yum.repos.d/*.repo "$target"/etc/yum.repos.d/

if [[ -n "$install_groups" ]]; then
	yum -c "$yum_config" --installroot="$target" --releasever=/ --setopt=tsflags=nodocs \
		--setopt=group_package_types=mandatory -y groupinstall `echo "${install_groups[@]}"`
fi

if [[ -n "$install_packages" ]]; then
	yum -c "$yum_config" --installroot="$target" --releasever=/ --setopt=tsflags=nodocs \
		--setopt=group_package_types=mandatory -y install `echo "${install_packages[@]}"`
fi
